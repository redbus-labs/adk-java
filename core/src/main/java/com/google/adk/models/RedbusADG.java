...
public class RedbusADG extends BaseLlm {
     ...
     * @return The cleaned string, containing only allowed characters. Returns null if the input was null.
     */
...
// Add the generated tool JSON object to your functions list/array
        ...
        String funcresp=functions.toString(1);
    

        String modelId =this.model();// "devstral";//"llama3.2:3b-instruct-q2_K";//"llama3.2"; // The 1b doesn't support tool
 

        JSONObject agentresponse = callLLMChat( modelId,messages, functions.length()>0?functions:null );//Tools/functions can not be of 0 length
       ...
       LlmResponse.Builder responseBuilder = LlmResponse.builder();
        ...
        //Call tool
            ...
            if (responseQuantum.getString("finish_reason").contentEquals("function_call")) {
     ...
     * Fetches username and password from environment variables.
     *
     * @param model
